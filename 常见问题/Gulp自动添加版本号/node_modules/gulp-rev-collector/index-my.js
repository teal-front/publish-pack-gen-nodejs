...
function replaceVersion(name){
    var endA = name.split('.'),
        endLen = endA.length - 1,
        endRep = name.replace('.' + endA[endLen], ''),
        startA = name.split('-'),
        startRep = endRep.replace(startA[0] + '-', '');
    return startRep;
}
        ...
        for (var key in manifest) {
            ...
            if (opts.replaceReved) {
                patterns.push( escPathPattern( (path.dirname(key) === '.' ? '' : closeDirBySep(path.dirname(key)) ) + path.basename(key, path.extname(key)) ) 
                            + opts.revSuffix 
                            + escPathPattern( path.extname(key) + 'v=1' )
                        );
            }
            ...
            if ( dirReplacements.length ) {
                    ...
                    patterns.forEach(function (pattern) {
                        changes.push({
                            ...
                            replacement: _.isFunction(dirRule.dirRpl) 
                                            ? dirRule.dirRpl(manifest[key]) + '?v=2'
                                            : closeDirBySep(dirRule.dirRpl) + manifest[key] + '?v=3'
                        });
                    });
                });
            } else {
                patterns.forEach(function (pattern) {
                    changes.push({
                        ...
                        replacement: manifest[key] + '?v=' + replaceVersion(manifest[key])
                    });
                });
            }
        }
